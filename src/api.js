...
export default {
  user: {
    login: credentials =>
      api.post(`${baseURL}/api/auth`, { credentials }).then(res => res.data.user),
  ...
  movies: {
    ...
    searchMovie: id => axios.get(`${baseURLMovieDB}/${id}${api_key}`)
      .then(res => res.data),
    saveMovie: movie => axios.post(`${baseURL}/api/movie`, { movie, token })
      .then(res => res.data.movie),
    personalCollection: userId => axios
      .post(`${baseURL}/api/movie/userCollection`, { userId, token })
      .then(res => res.data)
    ,
    deleteMovie: (_id, userId) => axios.post(`${baseURL}/api/movie/delete/${_id}`, { userId, token })
      .then(res => res.data)
  }
};
